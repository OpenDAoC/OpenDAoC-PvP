...
namespace DOL.GS.Quests
{
	public class PlayerKillAny : DailyQuest
	{
		...
		// Constructors
		public PlayerKillAny() : base()
		{
		}

		public PlayerKillAny(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public PlayerKillAny(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public PlayerKillAny(GamePlayer questingPlayer, DBQuest dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		{
			if (!DOL.GS.ServerProperties.Properties.LOAD_QUESTS)
				return;
			...
			ReyAlb.AddQuestToGive(typeof (PlayerKillAny));
		...
		public static void ScriptUnloaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			ReyAlb.RemoveQuestToGive(typeof (PlayerKillAny));
		}
		...
		protected static void TalkToRey(DOLEvent e, object sender, EventArgs args)
		{
			...
			if(ReyAlb.CanGiveQuest(typeof (PlayerKillAny), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			PlayerKillAny quest = player.IsDoingQuest(typeof (PlayerKillAny)) as PlayerKillAny;
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					...
					{
						case "help me out":
							player.Out.SendQuestSubscribeCommand(ReyAlb, QuestMgr.GetIDForQuestType(typeof(PlayerKillAny)), "Will you undertake " + questTitle + "?");
							break;
					}
				}
				else
		...
		private static void SubscribeQuest(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(PlayerKillAny)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!ReyAlb.GiveQuest(typeof (PlayerKillAny), player, 1))
					return;
